...
CREATE TABLE IF NOT EXISTS `user_setting` (
    ...
    `enable_qq_notification` TINYINT NOT NULL DEFAULT 0 COMMENT '启用QQ通知',
    `notification_email` VARCHAR(255) DEFAULT NULL COMMENT '通知接收邮箱',
    `notification_phone` VARCHAR(50) DEFAULT NULL COMMENT '通知接收手机号',
    `notification_qq` VARCHAR(50) DEFAULT NULL COMMENT '通知接收QQ号',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `reminder_rule` (
    ...
    `channel` VARCHAR(20) NOT NULL DEFAULT 'IN_APP' COMMENT '提醒渠道 IN_APP/LOCAL_APP/BROWSER/EMAIL/SMS/QQ',
