...
public class AiPromptBuilder {
    ...
    private String buildPromptContent(String currentTime, String timezone, String sourceType, String text) {
        return "你是一个专业的时间管理与任务规划助手。请根据用户提供的原始文本，提取并分析其中包含的任务、事件、风险等信息。\n\n" +
            ...
            "8. 相对时间解析规则（基于当前日期时间计算）：\\\"本周一\\\"~\\\"本周日\\\"对应当前周的周一到周日；\\\"下周一\\\"~\\\"下周日\\\"对应当前周的下一周；\\\"昨天\\\"=\\\"当前日期-1天\\\"；\\\"今天\\\"=\\\"当前日期\\\"；\\\"明天\\\"=\\\"当前日期+1天\\\"；\\\"后天\\\"=\\\"当前日期+2天\\\"；\\\"大后天\\\"=\\\"当前日期+3天\\\"；\\\"X天后\\\"=\\\"当前日期+X天\\\"；\\\"下周五\\\"=\\\"下一周的周五\\\"；时间表达式如\\\"8:00\\\"默认为当天或最近的合理日期，结合上下文判断。\n" +
            "9. checklist 必须包含具体可操作的检查项，例如\"将源码打包为zip格式，命名：组号_项目名_源码.zip\"，不要笼统写\"提交源码\"\n" +
            "10. title 必须包含课程或项目名称，例如\"《软件项目开发与实践》课程项目材料提交\"\n" +
            "11. description 必须包含完整的要求，例如提交格式、命名规范、邮箱地址、截止时间等全部细节\n" +
            "12. **任务合并规则（重要）**：同一份公告/通知中，相同截止时间和同一场景的多项要求，合并为 **一个任务**，用 checklist 区分不同交付物。请严格遵循上述合并规则。\n" +
            "13. **事件提取规则**：答辩、会议、演讲等有时间地点的事件提取到 events 数组中\n" +
            "14. **事件必须同时生成为任务**：每个 events 中的事件，必须同时创建一个对应的 task，deadline 设为事件开始时间";
    }
}
